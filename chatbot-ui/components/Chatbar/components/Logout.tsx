...
import { unsetToken } from '@/pages/api/auth/js-cookie';
  ...
  return isConfirming ? (
    <div className="flex w-full cursor-pointer items-center rounded-lg py-3 px-3 hover:bg-gray-500/10">
      ...
      <div className="flex w-[40px]">
        <IconCheck
          className="ml-auto mr-1 min-w-[20px] text-white dark:text-white hover:text-neutral-100"
          size={18}
          onClick={(e) => {
            e.stopPropagation();
        ...
        <IconX
          className="ml-auto min-w-[20px] text-white dark:text-white hover:text-neutral-100"
          size={18}
          onClick={(e) => {
            e.stopPropagation();
            setIsConfirming(false);
